...
def get_youtube_link(movie):
    ...
    f = urllib.urlopen(
        'https://www.youtube.com/results?search_query='+query+'+trailer').read()
        ...
        if request.method == 'POST':
            ...
            if '-w' in request.POST['search']:
                ...
                if not Q:
                    ...
                    Q = Q[0]
                    list1 = Movie.objects.filter(
                        Name__contains=Q, Watched=False)
                    list2 = Movie.objects.filter(
                        Year__contains=Q, Watched=False)
                    list3 = Movie.objects.filter(
                        Genre__contains=Q, Watched=False)
            else:
